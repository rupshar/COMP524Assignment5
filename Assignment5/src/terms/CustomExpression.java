...
public class CustomExpression extends BasicExpression implements SExpression {
	...
	public String toStringAsList() {
		...
		StringBuilder new_string = toStringAsListHelper(tail, ret_str);
		new_string.append(")");
		return new_string.toString();
	}
	
	public StringBuilder toStringAsListHelper(SExpression tail, StringBuilder str) {
		if(!(isNil(tail))) {
			str.append(" ");
			str.append(tail.getHead().toString());
			toStringAsListHelper(tail.getTail(), str);
		}
		return str;
	}
